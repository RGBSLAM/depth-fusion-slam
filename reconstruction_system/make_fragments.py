...
def read_rgbd_image(color_file, depth_file, convert_rgb_to_intensity, config):
    ...
    # print("@@@@@@@@@@@@@@@@@@@@@@ max depth: ", config["max_depth"], "@@@@@@@@@@@@@@@@@@@@@@@@@@@@2")
    return rgbd_image
...
def integrate_rgb_frames_for_fragment(color_files, depth_files, fragment_id,
    ...
    print("pose_graph_name", pose_graph_name)
    print("pose graph:", pose_graph)
    print("pose_graph_node: ", len(pose_graph.nodes)) # 100
    # print("color_files:", color_files)
    print("color_files len: ", len(color_files))
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    # intrinsic = o3d.camera.PinholeCameraIntrinsic(1280, 720, 526.9379272460938, 526.9379272460938, 616.2449951171875, 359.21929931640625)
...
def make_pointcloud_for_fragment(path_dataset, color_files, depth_files,
    ...
    mesh = integrate_rgb_frames_for_fragment(
        ...
        join(path_dataset,
             config["template_fragment_posegraph_optimized"] % fragment_id), # fragments/fragment_optimized_%03d.json
        intrinsic, config)
    pcd = o3d.geometry.PointCloud()
    pcd.points = mesh.vertices
    pcd.colors = mesh.vertex_colors
    pcd_name = join(path_dataset,
                    config["template_fragment_pointcloud"] % fragment_id) #fragments/fragment_%03d.ply
    print("make .ply works!")
    print("pcd: ", pcd)
    print("pcd.points:", pcd.points)
    print("pcd.colors:", pcd.colors)
...
def process_single_fragment(fragment_id, color_files, depth_files, n_files,
    ...
    if config["path_intrinsic"]:
        # print("config:", config["path_intrinsic"])
        # intrinsic = o3d.io.read_pinhole_camera_intrinsic(
        #     config["path_intrinsic"])
        intrinsic =  o3d.camera.PinholeCameraIntrinsic(width = 1280, 
                                                         height = 720, 
                                                         fx = 526.9379272460938, 
                                                         fy = 526.9379272460938, 
                                                         cx = 616.2449951171875,
                                                         cy = 359.21929931640625) # highbay
        ...
        # intrinsic =  o3d.camera.PinholeCameraIntrinsic(width = 1280, 
        #                                             height = 720, 
        #                                             fx = 607.46942138671875, 
        #                                             fy = 607.4534912109375, 
        #                                             cx = 636.9967041015625,
        #                                             cy = 369.26895141601562) # apartment
    else:
    ...
    make_posegraph_for_fragment(config["path_dataset"], sid, eid, color_files,
                                depth_files, fragment_id, n_fragments,
                                intrinsic, with_opencv, config)
    optimize_posegraph_for_fragment(config["path_dataset"], fragment_id, config) # COMMENT
...
def run(config):
    ...
    # print("config['n_frames_per_fragment']:", config['n_frames_per_fragment'], "config['folder_fragment']:", config["folder_fragment"]) # config['n_frames_per_fragment']: 100 config['folder_fragment']: fragments/
    
    make_clean_folder(join(config["path_dataset"], config["folder_fragment"])) # COMMENT
    ...
    if config["python_multi_threading"] is True:
        ...
        for fragment_id in range(n_fragments): # 3
